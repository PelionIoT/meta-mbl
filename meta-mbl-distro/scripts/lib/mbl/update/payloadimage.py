...
class PayloadImage(abc.ABC):
    ...
    @classmethod
    def register(cls):
        """Register a subclass with the versioned class registry."""
        vcr.register_versioned_class(
            cls.IMAGE_BASE_TYPE, cls.IMAGE_FORMAT_VERSION, cls
        )
    ...
    def image_type(self):
        ...
        The image's "type" is the name of the swupdate handler used to install
        the image, and includes the image's format version suffix. E.g.
        "ROOTFSv3".
        """
        return "{}v{}".format(self.IMAGE_BASE_TYPE, self.IMAGE_FORMAT_VERSION)

    @property
    def image_base_type(self):
        """
        Get the image's "base type".

        The image's "base type" is the name of the swupdate handler used to
        install the image, with the format version suffix removed. E.g.
        "ROOTFS".
        """
        return self.IMAGE_BASE_TYPE
