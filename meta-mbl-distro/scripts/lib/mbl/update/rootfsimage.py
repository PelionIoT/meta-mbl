...
class _Ext4GzRootfsImage(_RootfsImageBase):
    """Class for creating .ext4.gz image files for rootfs partitions."""

    IMAGE_COMPRESSION_TYPE = "zlib"

    def __init__(self, image_name, deploy_dir, tinfoil):
        """
        Create a _Ext4GzRootfsImage object.
    ...
    def stage(self, staging_dir):
        """Implement method from PayloadImage ABC."""
        assert len(self.archived_paths) == 1
        upi.stage_single_file_with_gzip_compression(
            staging_dir,
            uutil.ArchivedFileSpec(self._path, self.archived_paths[0]),
        )
    ...
    def archived_paths(self):
        """Implement method from PayloadImage ABC."""
        return [pathlib.Path("{}.ext4.gz".format(self.image_type))]


class RootfsImageV4(_Ext4GzRootfsImage):
