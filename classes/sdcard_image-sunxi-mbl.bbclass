...
IMAGE_CMD_sunxi-sdimg () {
	...
	mcopy -i ${WORKDIR}/boot.img -s ${DEPLOY_DIR_IMAGE}/kernel.itb ::
	mcopy -i ${WORKDIR}/boot.img -s ${DEPLOY_DIR_IMAGE}/kernel.its ::
	mcopy -i ${WORKDIR}/boot.img -s ${DEPLOY_DIR_IMAGE}/sunxi-spl.bin ::
	mcopy -i ${WORKDIR}/boot.img -s ${DEPLOY_DIR_IMAGE}/u-boot-nodtb.bin ::
	mcopy -i ${WORKDIR}/boot.img -s ${DEPLOY_DIR_IMAGE}/u-boot.dtb ::
	mcopy -i ${WORKDIR}/boot.img -s ${DEPLOY_DIR_IMAGE}/u-boot.its ::
	mcopy -i ${WORKDIR}/boot.img -s ${DEPLOY_DIR_IMAGE}/boot.cmd ::
	mcopy -i ${WORKDIR}/boot.img -s ${DEPLOY_DIR_IMAGE}/boot.its ::
	...
	dd if=${DEPLOY_DIR_IMAGE}/u-boot-signed.itb of=${SDIMG} bs=1024 seek=40 conv=notrunc
}
